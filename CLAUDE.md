...
The v2 revision is applied in halves. The shell (section 6) was built in 2C. The first half of the migration (2026-09-24) applied the tokens, the state mark, the type budget as a lint, the copy rules, the market inks and the removal of the anchor; `globals.css` holds section 1 and nothing of the first palette. The second half is the market screen: the pinned sheet (3.24), the odds line replacing the tile grid (3.13, 3.22) with every number in percent, and the link tiles (3.27).
...
- **Every market has an ink** (docs/design.md 1.8, `src/lib/ui/ink.ts`): decided once when it is asked (the mark's hue, else a hash of the id, balanced against the questions still open between the same people), stored in `dares.ink` with its source, and picked by the creator behind More. On the market's own screen `inkVars` swaps `--ground`, `--surface`, `--line` and `--field` for its layers on the screen's root and paints the grain over its ground; nothing else is tinted, and everywhere else the ink is only the stamp behind the mark. A row with no stored ink reads as its hash. The emoji hue table is `src/lib/ui/emoji-hues.ts`, generated, never edited by hand.
- **The anchor is gone from the schema too.** `anchor_value`, `anchor_at` and `anchor_rationale` are out of `src/db/schema.ts`; migration `0015_drop_anchor` drops them and is applied only after a build that never reads them is deployed.
- No em dashes in any generated documentation or copy.

## Current phase

**The v2 migration.** Submission gate October 13, 2026, 23:59 ET. Applying the revised design (`docs/design.md`, 2026-09-24) across the app, in two halves with a check-in at the seam. First half, done: tokens, the state mark, the type budget as a lint, copy per 4.6 and 4.9, market inks, the anchor removed. Second half, next: the pinned sheet (3.24) where the ballot, the arbitration panel and the ask flow's primary become moves; the odds line replacing the tile grid (3.13, 3.22) with every number in percent; the link tiles (3.27), the result tile doubling as the ruling share card. Deferred deliberately: stickers (after the media pipeline), number entry (3.26, with the range rule), calibration on You.
