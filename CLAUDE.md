...
The v2 revision is applied in two steps. The shell (section 6) was built in 2C. The palette, the type budget, the state marks, the market inks and the odds line replacing the tile grid are the next phase, so until then `globals.css` keeps the first palette's names and values (marigold is still `--primary`), and every new screen is built against tokens, never values, so that migration stays the find-and-replace the document's migration table describes.
...
**Phase 2C: the settler.** Submission gate October 13, 2026, 23:59 ET. The design audit landed on 2026-09-24 as v2 of `docs/design.md`; its section 6 replaced the navigation 2C opened with (a Home bar) with three roots, Start and the back control. Applying the rest of v2 across the app (palette, type budget, state marks, market inks, the odds line) is the next phase, not this one.

2C built: a way home under the thumb on every screen but home; "Needs you" ranked by what is time-bound; the ballot as the screen when it is yours to cast; arguments (the same market at a different pace), the three-tier triage with criterion selection and the refusal enforced in code, careful mode, the stalemate rule chosen at asking and shown before anyone is in, arbitration with a ruling hashed onchain, expiry as its own ending, and one scheduler (pg_cron, once a minute, one secret-authed door) for the deadline notice, auto-lock, `expire()` and the arbitration backstop.

Verified by two real accounts on the real chain: an interpersonal dispute declined with a dare offered; a contestable claim given three criteria, one chosen, ruled against it (softly); a deadlock taken to arbitration with a case stated, the ruling's hash matching the contract and the indexer; a checkable argument settled by agreement with the loser's whole stake on both timelines. Not yet exercised in a real session: the second person's entry through the new argument screen (their login exists only on production, which runs the old build until deploy), careful mode end to end, and expiry.

Out of scope, deliberately: the profile (clean-resolution rate and calibration have no designed home yet); two-party criterion agreement; ghosts in arguments.

**Checkpoint (needs people, on production):** an argument settled in under a minute between two people, the loser's obligation on both timelines; a contestable claim with criteria proposed, one chosen, ruled against it; an interpersonal dispute declined with the dare offered; a question that deadlocks and reaches arbitration with statements from both sides and a ruling recorded against the chain.
