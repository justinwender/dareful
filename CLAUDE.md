...
- There is a way home under the thumb on every screen but home (`HomeBar`), and sticky action areas sit above it. "Needs you" orders by what is time-bound, as priority and never as pressure: no countdowns, no day counts, no ageing.
...
**Phase 2C: the settler.** Submission gate October 13, 2026, 23:59 ET. A design system audit and a navigation architecture are landing mid-phase and will change `docs/design.md`; when they arrive, stop and read them. Applying them across the app is the next phase, not this one.
