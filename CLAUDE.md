...
- `docs/marks-and-memories.md`: marks (an emoji, a picture or a sticker a creator attaches to a market or a denomination), the ink a market's mark decides, and media on events. Changes the schema: `media` replaces `photos`, marks live on `dares` and `denominations`, `mark_kind` gains `sticker`, and a market stores its `ink` and `ink_source`.
- `docs/design.md`: the design specification, revised on 2026-09-24 (v2). Tokens as literal values, the two propagating rationales (obligation direction without profit-and-loss color; the visual language for denominations), component states, the shell (section 6: three destinations and one button), and the rules for deriving screens nobody drew. "What changed in this revision" at its top lists every change that touches code.
...
- Safe-area insets: top and sides are paid once on `body`; anything fixed or sticky pays its own bottom inset. Never `100vh`. `Screen` paints a fixed band the height of the top inset behind the translucent status bar (the ground with its grain; a market's ground on a market screen, which is why it is in `Screen` and not the layout). `ViewportRepair` (root layout, iOS only) scrolls a pixel and back when the visual viewport is left short or offset after the keyboard, an iOS 26.0 regression that makes fixed elements sit high and drift (docs/testing.md session 7); the rule is `viewportStuck`.
...
- **The nine type tokens are the only text sizes** (docs/design.md 1.2): `text-serif-xl`, `text-serif-l`, `text-serif-m`, `text-numeral-hero`, `text-numeral` (and `-sm`), `text-body` (and `-strong`), `text-body-sm`, `text-label`, `text-caption`. Control text is outside the count: the utilities named without the `text-` prefix (`link-tertiary`, `link-row`, `chip-text`, `chip-context`, `tab-text`), and any token on a button, a form field, its label or a chip, which the lint places by the element that carries it (`--explain` lists every reference). `npm run lint:type` (also before every build) refuses a legacy token or a literal pixel size anywhere else, and holds every screen to four tokens and one serif size, or to `scripts/type-budget.baseline.json` where a screen was over on the day the rule arrived. Never add a screen to the baseline; burn one down. With controls out, eleven screens are still over four: that contradiction between the budget and the specification's own inventory is the design session's, not a reason to relax the lint.
...
- **Every market has an ink** (docs/design.md 1.8, `src/lib/ui/ink.ts`): decided once when it is asked (the mark's hue with reds under 40 folding into Rose, else a hash of the id, balanced against the questions still open between the same people), stored in `dares.ink` with its source, and picked by the creator behind More. On the market's own screen `inkVars` swaps `--ground`, `--surface`, `--line` and `--field` for its layers on the screen's root and paints the grain over its ground; nothing else is tinted, and everywhere else the ink is only the stamp behind the mark. A row with no stored ink reads as its hash. The emoji hue table is `src/lib/ui/emoji-hues.ts`, generated, never edited by hand.
