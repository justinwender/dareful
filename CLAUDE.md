...
- `docs/marks-and-memories.md`: marks (an emoji, a picture or a sticker a creator attaches to a market or a denomination), the ink a market's mark decides (the emoji ink table, `src/lib/ui/emoji-inks.json`, made by `scripts/emoji-inks.py`), the mark picker, and media on events. Changes the schema: `media` replaces `photos`, marks live on `dares` and `denominations`, `mark_kind` gains `sticker`, and a market stores its `ink` and `ink_source`.
- `docs/design.md`: the design specification, revised on 2026-09-25 (the fifth design session; v2 was 2026-09-24). Tokens as literal values, the two propagating rationales (obligation direction without profit-and-loss color; the visual language for denominations), component states, the shell (section 6: three destinations and one button), and the rules for deriving screens nobody drew. "What changed in this revision" at its top lists every change that touches code.
- `docs/design/reference/design.html`: the exported design. Source of truth for values and layout intent, never copied as markup.
...
`docs/design.md` is the specification and `docs/design/reference/design.html` is the export. Port values and layout intent from the export into Tailwind and shadcn components; never copy its markup. The tokens live in `src/app/globals.css` as CSS custom properties and the Tailwind theme reads from them, so a screen built in Phase 5 cannot drift from one built in Phase 1. Dark is the only shipped theme. Young Serif for a question, an outcome, or one number that matters; Hanken Grotesk for everything else; 13px is the floor.

The fifth design session (2026-09-25) counts the type budget in sizes, ships the emoji ink table, designs the mark picker (3.29) and specifies number markets; of those, the lint, the table, the leaderboard rank and the unit mark are built, and the picker and number markets wait. The v2 revision before it was applied in halves. The shell (section 6) was built in 2C. The first half of the migration (2026-09-24) applied the tokens, the state mark, the type budget as a lint, the copy rules, the market inks and the removal of the anchor; `globals.css` holds section 1 and nothing of the first palette. The second half (2026-09-25) is the market screen: the pinned sheet (3.24) on every market and task screen, the odds line replacing the tile grid (3.13, 3.22) with every number in percent, the call sheet, the settled sheet and the link tiles (3.27). Still undrawn or deferred: stickers, number entry (3.26), the tile with a photo, calibration on You, and the cover form's primary in the sheet.
...
- `SUPABASE_URL` and `SUPABASE_SECRET_KEY`: server-only, for Storage (the private `media` bucket, over the Storage REST API with the modern `sb_secret_…` key, which rotates on its own). Nothing reads the legacy `service_role` or anon keys, so both can be disabled. There is no `NEXT_PUBLIC_SUPABASE_*` variable and there never will be.
...
- `TICK_SECRET`: the scheduler's bearer token, the same value in Vercel and in Supabase Vault (`dareful_tick_secret`); rotate both together. Setup in `src/db/ops/scheduler.sql`.
- `OPS_EMAIL`: where the tick writes, at the top of each hour, while the relayer holds under 3 MON (`src/lib/chain/watch.ts`). Off when blank. The relayer is shared with production, so a local test run spends its gas (about half a MON for the database suites).
...
- The browser gets no Supabase connection: no anon key, no `NEXT_PUBLIC_SUPABASE_ANON_KEY`, no client-side Supabase SDK. A photo reaches the browser only as a signed URL good for a minute, issued by `/api/media/[id]` after the check of who is asking.
...
- Now is event-first (docs/design.md 4.7): "Needs you", "Running", "Just happened", in that order, each gone when empty. It routes rather than does: nothing on it starts anything (that is Start), lists people (that is People) or leaves the product (that is You). "Needs you" holds only what this person can finish now and never counts, badges, or says how long anything has waited; a running row says where a question stands and carries no action.
...
- Errors are ink, never red (docs/design.md 5.1); focus is the global 2px ink outline, 2px outside the control, never a ring class: `Problem` under the field with `aria-invalid` and `aria-describedby`, and `ProblemSummary` once above the button. Buttons wait per 5.2: the label stays, a 2px line runs, "Still going." at three seconds.
...
- Safe-area insets: top and sides are paid once on `body`; anything fixed or sticky pays its own bottom inset as padding, at `bottom: 0` (the tab bar, the pinned sheet); only the Start button is positioned by the inset, because it must clear the bar. Never `100vh`. `Screen` paints a fixed band the height of the top inset behind the translucent status bar (the ground with its grain; a market's ground on a market screen, which is why it is in `Screen` and not the layout). The tab bar sits high with a band beneath it in the installed app and nowhere else; the cause is not yet known (a cold start with no keyboard showed it, which removed the viewport repair built for the keyboard), and `ViewportProbe` behind "Measure the screen" on You prints the numbers that decide it (docs/testing.md session 9). The probe leaves with the cause.
...
- **The settler refuses before it rules.** Triage comes first and matters more than the ruling. An interpersonal dispute, a matter of taste, anything with no claim about the world, and a contestable claim with no measurable criterion are declined in code (`declined()`), whatever a model said, with an offer to make it a dare. With no triage there is no settler: a failed model call is never permission to rule. A refusal never comments on either person.
- A contestable claim's criterion is inside `terms_text`, so inside the hash, and on the card and the invitation before anyone is in. So is the stalemate rule. Entering accepts both; that consent is in the entry signature, and it is the only reason arbitration is fair.
- An argument signs `resolvesBy = 0` (due at once); its offchain `resolves_by` is the moment the second person got in. It is between two people, locks the moment both are in, and its default sides are all the way.
- A case (`dare_statements.kind = 'statement'`) and what happened (`'update'`) are never mixed, and only someone who is in may state a case. The ruling is stored exactly as hashed: any cleanup or clipping happens before `keccak256`, never after.
- Expired is not void: `resolved_by = 'expired'`, no outcome, nothing minted, no toll. A void by quorum or by the arbitrator counts against whoever wrote the terms.
- The scheduler is one secret-authed door, `/api/tick`, called by pg_cron once a minute. Every job in it is idempotent and also reachable by a person, so a missed tick delays and never breaks. It also reads the relayer's balance after its jobs and reports it; under the floor it warns every minute and emails `OPS_EMAIL` hourly, and a balance it cannot read never fails the tick. **The development machine shares the production database: never call the tick unscoped from a test or a script.** Tests pass `onlyIds`.
- Display-only prose from a model is clipped, never a reason to throw away the answer it came with. Generated text has em dashes removed before it is stored or hashed.
- **The shell is three roots and one button** (docs/design.md section 6). Now, People and You each render the tab bar (`TabBar`) themselves, so no other screen has it. Every non-root screen has the 48px back control at its top left (`TopBar back`), which lands on the root it came from (`rootFor`: never a remembered address that is not a root, never browser history); reached from a link while signed out, a screen shows the wordmark instead. Start is the one chalk control on a root and opens the one sheet of beginnings; it is hidden on an empty Now. A modal sheet closes with its 48px close and by dragging its handle down. The citron dot on Now means time-bound and nothing else: no number, ever. "Needs you" orders by what is time-bound, as priority and never as pressure: no countdowns, no day counts, no ageing.
- The AI anchor is dead (docs/decisions.md 2026-09-24): an AI-suggested number contaminates the calibration score, because people clustering around it are measured on the AI's calibration rather than their own. `anchor_value`, `anchor_rationale` and everything that generates or shows them come out in the migration phase; nothing new reads them.
- A number market's scale is fixed when it is asked (by the asker, or by the AI when left blank), stored in `range`, and shown to entrants only when the asker chose it. Never derived from entries: one extreme entry would inflate it, compress everyone else's transfers, and under truncation collapse the market. Entries are not clamped; anything far outside floors at zero.
...
- **The nine type tokens are the only text, and the budget counts sizes** (docs/design.md 1.2, 4.8): `text-serif-xl`, `text-serif-l`, `text-serif-m`, `text-numeral-hero`, `text-numeral` (and `-sm`), `text-body` (and `-strong`), `text-body-sm`, `text-label`, `text-caption`. A size is a family at a pixel size and weight is free inside it (`label` and `caption` are one size; `body-sm` and `numeral` 15 are one size): at most four sizes a screen, one of them serif, and at most two weights at a size, 400 and 600. Not counted: text inside a control (by element: buttons, fields, labels, chips; by 4.8's marker `data-type-exempt` on the code boxes; by file: the button, chip, tab bar, odds line and obligation token components), and the utilities named without the `text-` prefix. `npm run lint:type` (also before every build) refuses a legacy token or a literal pixel size anywhere else and holds every screen to the budget, or to `scripts/type-budget.baseline.json` where a screen was over on the day the size rule arrived (2026-09-25: Now, the claimant screen and the ask flow, with the reasons in docs/decisions.md). Never add a screen to the baseline; burn one down. `--explain` lists every reference with its size, weight and element.
- **State is a mark, never a sentence** (docs/design.md 3.23): `StateMark` at the head of a kicker or a meta line, with the citron `LiveDot` before it only when a clock is waiting on this person. The only words beside a mark are a clock or, on a needs-you row, the reason the row exists. "Waiting on an answer", "Settled", "Squared up", "Everyone's in" and their kind never come back as text.
- **Every market has an ink** (docs/design.md 1.8, `src/lib/ui/ink.ts`): decided once when it is asked (the mark's ink from the emoji table, else a hash of the id, balanced against the questions still open between the same people), stored in `dares.ink` with its source, and picked by the creator behind More. On the market's own screen `inkVars` swaps `--ground`, `--surface`, `--line` and `--field` for its layers on the screen's root and paints the grain over its ground; nothing else is tinted, and everywhere else the ink is only the stamp behind the mark. A row with no stored ink reads as its hash. **The emoji table is the renderer's** (`src/lib/ui/emoji-inks.json`, read by `emoji-ink.ts`): every emoji Noto Color Emoji can draw, mapped straight to an ink or to null (templates and greys hash), keys without variation selectors or skin tones. It was computed from the font the link tiles are drawn with, so a market's colour is what its share card draws; a mark that is not a key is refused at draft. Reds fold into Rose inside the table; `nearestInk` keeps the same folds (345 to 20 always, 20 to 40 at chroma 0.14 and up, greens 120 to 165 into Olive) for pictures measured at upload. Regenerate only with `scripts/emoji-inks.py` when the renderer's font changes; never edit it by hand. A unit's mark never takes an ink: bare inside a token, on `--surface-2` elsewhere.
- **The anchor is gone from the schema too.** `anchor_value`, `anchor_at` and `anchor_rationale` are out of `src/db/schema.ts`; migration `0015_drop_anchor` dropped them once the first-half build was deployed (2026-09-25).
- **Every market screen and every task screen keeps its one move in the pinned sheet** (`PinnedSheet`, docs/design.md 3.24): not modal, never empty while the market runs, two heights only when there is more to show, and only its grabber and header row drag. Its room is paid by `Screen` through `--sheet-room`. A moment that binds other people (a vote, asking for the tiebreaker) still gets the modal `Sheet` on top. On a market: `MarketStage` (the odds line, then the link), `CallSheet` (say what happened, call it, split), `SettledSheet` (send how it ended); nothing on a voided or expired one.
- **The odds line and the weight line are one object** (3.13, 3.22): ten buckets, a value that is null until touched (nothing starts at 50%), every number a whole percent (`percentOf`), stakes travelling as decimal strings, heights recomputed in integers on the client so the picture follows a change before it is saved. The group's marker never moves before the save: that would need everyone's numbers.
- **The words around a tiebreaker credit the agreement, never the app**: "the tiebreaker everyone agreed to", on the details, the split sheet, the ruling section and the result tile. Never "the app called it".
- **The link tiles are drawn, not the share card** (`src/lib/ui/tiles.tsx`, 3.27): the asking tile until settled, the result tile after, from one route; the asker's first name, the mechanic and an absolute close time in the asker's zone (`dares.zone`, kept from the display cookie at draft time; older rows say UTC and say so); never who is in, at what, or a relative time. Fonts are read from `src/lib/ui/fonts` and traced by `next.config.ts`; never `fetch(new URL(…, import.meta.url))`, which Turbopack cannot serve.
- **The lifecycle of an obligation is the creditor's, from the row** (docs/design.md 6.3; PLANNING.md 5a, 7): on the person view the creditor's covered card is the move (`CloseObligation`), and a consequence the creditor is owed inside a market's story is the same move in a row (the story card's link is the story; its consequences sit outside the link). The sheet holds two acts of equal standing, "Settled" (the photo moment, Principle 6) and "Call it even" (Principle 5), each closing everything still open on the obligation with a ledger-wallet signature over the obligation's own close counter (`src/lib/ledger/closes.ts`). Only `to_user` may close; the contract checks again. What is open is the smaller of the indexer's `remaining` and the chain's `minted - closed`, because the indexer runs seconds behind. The moment of the close is written to `obligations.closed_at` (the offchain clock, which is what "Just happened" orders by); settled or forgiven stays the chain's. A photo that fails never undoes a settlement. An obligation a market minted has an id the contract derived, with no RFC version or variant bits: a boundary that takes an obligation id uses `isUuidLike`, never `z.string().uuid()`.
- **Netting is the row under the person-view header**: one per unit in each set of people that goes both ways, "Cancel out the N each way", one signature from either party, one transaction, the contract cancelling by the smaller side (`nettablePairs`, `netBetween`). "Net" is a banned word on screen. An obligation a netting empties reads as settled on its card.
- **The other person hears when something closes** (Principle 1): settled and forgiven once per obligation, a netting once per pair per six hours; `notification_log` holds a question or an obligation or, for a netting, neither (migration 0017). A notice names who and the memo, never the unit or an amount.
- **The photo pipeline keeps two derivatives and nothing else** (`src/lib/media`): the 1080px frame and the 256px square, re-encoded so every field of EXIF is gone (GPS above all), orientation applied, only the colour profile kept, and `captured_at` read first from `DateTimeOriginal` with the camera's offset or the viewer's zone. The bucket is private (`src/db/ops/storage.sql`); `/api/media/[id]` answers 404 to everyone but the two people in the obligation (a photo nobody may see does not exist) and 302 to a sixty-second signed URL otherwise. The phone shrinks a photo over 3.5MB to 2048px first; server actions take 5MB.
- **The rally strip is a sequence, never a count** (docs/design.md 3.11): the last twelve unsettled pick-ups, oldest first, a row for them and a row for you, nothing under four. Square is computed from Envio's open edges (the People collapse row, the header's "even"), never declared. Closing the books is cut, superseded with the group as a place (docs/decisions.md 2026-09-25, "Three rulings"): if a closing moment returns it is person-level and designed.
- No em dashes in any generated documentation or copy.

## Current phase

**Lifecycle (built and deployed 2026-09-25).** Settling and forgiving from the obligation row and from a market story's consequence, with the settlement photo; netting from under the header; the rally strip; the photo pipeline behind a private bucket and one signed-URL door; notices for settled, forgiven and netted; closes in "Just happened" by `closed_at`. Email is live (Resend, the records in docs/decisions.md). Open: the band under the tab bar in the installed app, with the probe on You waiting for its numbers; the legacy Supabase keys, to be disabled by the operator now that the photo path is verified on production.

**The v2 migration.** Submission gate October 13, 2026, 23:59 ET. Applying the revised design (`docs/design.md`, 2026-09-24) across the app, in two halves with a check-in at the seam. First half, done and deployed: tokens, the state mark, the type budget as a lint, copy per 4.6 and 4.9, market inks, the anchor removed. Second half, built: the pinned sheet (3.24) where the ballot, the arbitration panel and the ask flow's primary became moves; the odds line replacing the tile grid (3.13, 3.22) with every number in percent; the link tiles (3.27), the result tile doubling as the ruling share card with the tiebreaker credited to the agreement. Deferred deliberately: stickers (after the media pipeline), number entry (3.26, with the range rule), the tile with a photo, calibration on You, the cover form's primary in the sheet.

2C, done and verified by two real accounts on the real chain: arguments as the same market at a different pace, the three-tier triage with the refusal enforced in code, careful mode, the stalemate rule chosen at asking, arbitration with a ruling hashed onchain, expiry as its own ending, and one scheduler (pg_cron once a minute through one secret-authed door). Still owed from 2C, through real sessions: the second person's entry through the argument screen, careful mode end to end, and expiry under the void rule; and the people checkpoint on production (an argument under a minute; a contestable claim with criteria; an interpersonal decline; a deadlock to arbitration).
