...
- Safe-area insets: top and sides are paid once on `body`; anything fixed or sticky pays its own bottom inset. Never `100vh`. `Screen` paints a fixed band the height of the top inset behind the translucent status bar (the ground with its grain; a market's ground on a market screen, which is why it is in `Screen` and not the layout). `ViewportRepair` (root layout, iOS only) scrolls a pixel and back when the visual viewport is left short or offset after the keyboard, which makes fixed elements sit high and drift. It fires on that symptom, never on an iOS version: the phone that showed the drift runs 26.6.2, so the iOS 26.0 regression first blamed is not the cause, and the cold-start check in docs/testing.md session 7 decides whether the mechanism holds. The rule is `viewportStuck`.
...
- **The lifecycle of an obligation is the creditor's, from the row** (docs/design.md 6.3; PLANNING.md 5a, 7): on the person view the creditor's covered card is the move (`CloseObligation`), and its sheet holds two acts of equal standing, "Settled" (the photo moment, Principle 6) and "Call it even" (Principle 5), each closing everything still open on the obligation with a ledger-wallet signature over the obligation's own close counter (`src/lib/ledger/closes.ts`). Only `to_user` may close; the contract checks again. What is open is the smaller of the indexer's `remaining` and the chain's `minted - closed`, because the indexer runs seconds behind. A photo that fails never undoes a settlement. An obligation a market minted has no row of its own yet and cannot be closed from the story: a known gap.
...
- **The rally strip is a sequence, never a count** (docs/design.md 3.11): the last twelve unsettled pick-ups, oldest first, a row for them and a row for you, nothing under four. Square is computed from Envio's open edges (the People collapse row, the header's "even"), never declared; closing the books has no home in the specification and is not built.
- No em dashes in any generated documentation or copy.

## Current phase

**Lifecycle (built 2026-09-25).** Settling and forgiving from the obligation row with the settlement photo, netting from under the header, the rally strip, the photo pipeline behind a private bucket and one signed-URL door, and notices for settled, forgiven and netted. Waiting on the operator: `SUPABASE_SECRET_KEY` in `.env.local` and Vercel (photos are off until then), the domain's email records, and a ruling on where closing the books lives, on a `closed_at` shadow for "Just happened", and on the netting row's placement and words. Known gaps: an obligation a market minted cannot be closed from its story yet.
