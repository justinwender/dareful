 ...
 */
export function Sparkline({ points, openedLabel, currentTenth }: { points: Array<{ at: number; percent: number }>; openedLabel: string; currentTenth: number }) {
  ...
  return (
    <figure className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-3">
        <svg viewBox="0 0 300 56" preserveAspectRatio="none" className="h-14 min-w-0 flex-1" role="img" aria-label={`The group's number since ${openedLabel}, now ${currentTenth} in 10`}>
        ...
        <span className="text-numeral-sm text-ink">{currentTenth} in 10</span>
      </div>
