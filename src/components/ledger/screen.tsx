...
export function Screen({ children, className }: { children: ReactNode; className?: string }) {
  return <main className={cn("mx-auto flex w-full max-w-[430px] flex-1 flex-col px-5 pb-8", className)}>{children}</main>;
}
...
/** Bottom action area: 16px top, 20px sides, 24px bottom. At most one primary button per viewport. */
export function ActionArea({ children }: { children: ReactNode }) {
  return <div className="sticky bottom-0 -mx-5 mt-auto bg-ground px-5 pt-4 pb-6">{children}</div>;
}
