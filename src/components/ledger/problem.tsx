...
/**
 * docs/design.md 5.1. There is no red, and marigold already means what happened, today, and the next tap, so an
 * error in marigold competes with the button at the moment someone most needs to find it. Ink carries errors,
 * on four channels: position (at the field, and once more above the button), weight, a glyph, and the words.
...
/** The border a field takes when it has a problem: 1px line becomes 1.5px ink. Focus stays 2px marigold. */
...
export function Problem({ message, id }: { message?: string | null; id?: string }) {
  ...
  return (
    <p id={id} className="flex gap-2 text-[15px] leading-[20px] text-ink">
      <AlertGlyph />
      <span>{message}</span>
    </p>
  );
...
export function ProblemSummary({ messages, children }: { messages: ReadonlyArray<string | null | undefined>; children?: ReactNode }) {
  ...
  return (
    <div role="alert" className="flex flex-col gap-2 rounded-[14px] border border-line-strong bg-surface-2 px-[14px] py-3">
      {list.map((m) => (
        <p key={m} className="flex gap-2 text-[15px] leading-[20px] text-ink">
          <AlertGlyph />
          <span>{m}</span>
        </p>
      ))}
