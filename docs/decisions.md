...
**The bar and the sheet sitting high, and the bar drifting on Now.** The report read as the bottom inset paid twice. It is paid once in the code: `body` pays top and sides only, the tab bar and the pinned sheet each pay their own bottom inset, and no other fixed element pays it. The grain was the first suspect for the drift, since a `filter` on an ancestor makes it a fixed element's containing block: the grain's filter is inside the SVG the background image is made of, not a CSS filter on the body, and the rendered page confirms no ancestor of the bar (main, body, html) carries a transform, filter, will-change, perspective, contain or backdrop-filter. What both symptoms match is a regression in iOS 26.0: after the keyboard has been up and gone, Safari leaves the visual viewport short and offset and fixed elements track it, sitting above the home indicator with the page beneath, and moving when the page scrolls up; the state lasts for the life of the document and so is carried by client-side navigation from the screen where something was typed (the ask flow, the code field on Now) to every screen after it, which is why Now on a cold start is fine and every screen after the ask flow is not. Apple has it fixed in 26.1 by user reports. `ViewportRepair` (in the root layout) listens for a field losing focus and for the visual viewport resizing, and when the reading says the viewport is stuck (`viewportStuck`: short or offset, nothing focused, no pinch zoom) scrolls one pixel and back, which makes Safari re-read the viewport. It only runs on iOS. Rejected: turning the app into a scroll container (`main` scrolling inside a fixed-height body with the bar and the sheet in flow), which sidesteps fixed positioning entirely but changes the scroll model of every screen (restoration, anchors, the sheet's own rule that it never locks the page) for a bug Apple has already fixed; and waiting for 26.1, which leaves everyone on 26.0 with a drifting bar.
